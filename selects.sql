...
SELECT SALES.DATE_OF_SALE, CAR_MODELS.NAME as "Model name", CAR_MARKS.NAME as "Mark name", COLORS.COLOR, CARS.PRICE, CLIENTS.NAME as "Client Name" FROM sales
...
SELECT EMPLOYEES.NAME, SALES.DATE_OF_SALE, CAR_MODELS.NAME as "Model name", CAR_MARKS.NAME as "Mark name", COLORS.COLOR, CARS.PRICE, CLIENTS.NAME as "Client Name" FROM sales
JOIN employees ON sales.employees_id = employees.id
JOIN CLIENTS ON sales.clients_id = clients.id
JOIN cars ON sales.cars_id = cars.id
JOIN car_models ON cars.car_models_id = car_models.id
JOIN car_marks ON car_models.car_marks_id = car_marks.id
JOIN colors ON cars.colors_id = colors.id
ORDER BY sales.DATE_OF_SALE ASC
FETCH FIRST 5 ROWS ONLY;

--cars bought by client
SELECT CLIENTS.NAME, CAR_MODELS.NAME as "Model name", CAR_MARKS.NAME as "Mark name", COLORS.COLOR, CARS.PRICE, SALES.DATE_OF_SALE, EMPLOYEES.Name as "Sold by" FROM sales
JOIN employees ON sales.employees_id = employees.id
JOIN CLIENTS ON sales.clients_id = clients.id
JOIN cars ON sales.cars_id = cars.id
JOIN car_models ON cars.car_models_id = car_models.id
